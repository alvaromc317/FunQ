...
test_that('rotate_scores_and_loadings function works', {
  ...
  expected_rotation = as.matrix(1.444052)
  ...
  expect_equal(round(results$rotation.matrix, 4), round(expected_rotation, 4))
})

test_that('FQPCA algorithm works', {
  set.seed(5)
  Y = matrix(rep(sin(seq(0, 2*pi, length.out=12)), 20), byrow=TRUE, nrow=20)
  Y = Y + matrix(rnorm(20*12, 0, 0.4), nrow=20)
  Y[sample(20*12, as.integer(0.2*20*12))] = NA
  ...
  expect_equal(round(results$loadings, 4), round(expected_loadings, 4))
  expect_equal(round(results$scores, 4), round(expected_scores, 4))
  expect_equal(results$n.iters, 4)
})
