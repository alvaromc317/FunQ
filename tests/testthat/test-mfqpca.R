...
test_that('MFQPCA accepts a data.frame with a tf column and group given as a column-name string', {
  data <- test_data_mfqpca()
  Y <- data[[1]]
  group <- data[[2]]
  # test_data_mfqpca() always seeds a handful of NAs; wrapping a matrix with
  # scattered NAs as a tfd_reg object emits an informational tidyfun message
  # about interpolation, unrelated to what this test is checking.
  Y.tf <- suppressWarnings(tf::tfd(Y, arg = seq(0, 1, length.out = ncol(Y))))
  df <- data.frame(y = Y.tf, subject = group)
  colnames(df)[1] <- "y"

  results <- suppressWarnings(mfqpca(data = df, group = "subject", colname = "y", npc.between = 1, npc.within = 1, quantile.value = 0.5, seed = 5, verbose = FALSE))
  expect_s3_class(results, "mfqpca_object")
  expect_true(is.matrix(results$loadings.between))
  expect_true(is.matrix(results$loadings.within))
})
...
test_that('MFQPCA error checking works', {
  ...
  expect_error(mfqpca(data=Y, group=group, max.iters=-1), "Expected an integer number >= 2")
  ...
  expect_error(mfqpca(data=df, group="a", colname="x"), "does not correspond to any column")
...
test_that('MFQPCA works with splines.method="quantreg"', {
  data <- test_data_mfqpca()
  Y <- data[[1]]
  group <- data[[2]]

  results <- mfqpca(data=Y, group=group, npc.between=1, npc.within=1, quantile.value=0.5, splines.method="quantreg", seed=5, verbose=FALSE)
  expect_s3_class(results, "mfqpca_object")
  expect_true(is.matrix(results$loadings.between))
  expect_true(is.matrix(results$loadings.within))
})
...
test_that('MFQPCA CV df function works', {
  data <- test_data_mfqpca()
  Y <- data[[1]]
  group <- data[[2]]
