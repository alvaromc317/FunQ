...
test_that("cross_validation_lambda incorrect inputs detection works", {
  Y = test_data_fqpca()
  expect_error(cross_validation_lambda(data=Y, lambda.grid=c(0, 1e-15), n.folds=3, verbose.cv=FALSE, splines.method='aaa')) # method
  expect_error(cross_validation_lambda(data=Y, lambda.grid=c(0, 1e-15), n.folds=2.5, verbose.cv=FALSE, splines.method='conquer')) # n.folds
})

test_that("cross_validation_lambda based on points function works", {
  Y = test_data_fqpca()
  cv_result = cross_validation_lambda(data=Y, lambda.grid=c(0, 1e-3), n.folds=3, verbose.cv=FALSE, splines.method='conquer', seed=5, return.models=FALSE)
...
test_that("cross_validation_df based on points function works", {
  Y = test_data_fqpca()
  cv_result = cross_validation_df(data=Y, quantile.value=0.9, splines.df.grid=c(5, 10), n.folds=3, verbose.cv=FALSE, splines.method='conquer', seed=5, return.models=FALSE)
