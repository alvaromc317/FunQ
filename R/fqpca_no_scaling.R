...
compute_F_Lambda_no_scaling = function(x, x_mask, quantile_value, basis_coef, spline_basis, spline_basis_i)
{
...
inner_loop_no_scaling = function(x, x_mask, Lambda0, spline_basis_i, quantile_value, lambda_ridge, R_block, solver)
{
...
new_fqpca_no_scaling <- function(loadings, scores, unnormalized_loadings, normalization_matrix,
                      spline_coefficients,
...
fqpca_no_scaling = function(x, n_components=2,  quantile_value=0.5, lambda_ridge=1e-12,  periodic=TRUE, splines_df=30, tol=1e-3, n_iters=30, solver='SCS', verbose=FALSE, seed=NULL)
{
  ...
  for(i in 1:n_iters)
  {
    ...
    loop_result = try(inner_loop_no_scaling(x=x, x_mask=x_mask, Lambda0=Lambda0, spline_basis_i=spline_basis_i, quantile_value=quantile_value, lambda_ridge=lambda_ridge, R_block=R_block, solver=solver), silent=TRUE)
  ...
  best_results = compute_F_Lambda_no_scaling(x=x, x_mask=x_mask, quantile_value=quantile_value, basis_coef=best_B, spline_basis=spline_basis, spline_basis_i=spline_basis_i)
