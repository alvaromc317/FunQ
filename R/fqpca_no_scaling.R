...
fqpca_no_scaling = function(x, n_components=2,  quantile_value=0.5, lambda_ridge=1e-12,  periodic=TRUE, splines_df=30, tol=1e-3, n_iters=30, solver='SCS', penalized=TRUE, method='br', verbose=FALSE, seed=NULL)
{
  ...
  for(i in 1:n_iters)
  {
    ...
    loop_result = try(inner_loop(x=x, x_mask=x_mask, Lambda0=Lambda0, spline_basis_i=spline_basis_i, quantile_value=quantile_value, lambda_ridge=lambda_ridge, R_block=R_block, solver=solver, penalized=penalized, method=method), silent=FALSE)
    error_chr = class(loop_result)
    if(error_chr=="try-error")
    {
      # If an error occurred (aka, algorithm failed to converge) exit the loop
      warning('Convergence on iteration ', i, ' failed. Providing results from previous iteration.')
